...
if PHOTONLIB_SUPPORTED:
    ...
    class PhotonLocalizer(commands2.Subsystem):
        ...
        def __init__(self, container, drivetrain, fieldLayoutFile: str, ignoreTagIDs=(),
                     importantTagIDs=(),
            ...
            for prefix in ["deploy/fields", ""]:
                candidate = join(prefix, fieldLayoutFile)
                print(f"PhotonLocalizer: trying field layout from {candidate}")
                if isfile(candidate):
            ...
            print(
                f"PhotonLocalizer: loaded field layout with {len(self.fieldLayout.getTags())} tags, from {fieldLayoutFile}")
        ...
        def initEnabledChooser(self):
            ...
            if self.username == "lvuser" and self.flippedFromAllianceColor is not None:
                ...
                print("PhotonLocalizer: color={}, flippedFromAllianceColor={}".format(color, flipped))
            print("PhotonLocalizer will assume flipped={} (username={}, rule={})".format(flipped, self.username,
                                                                                         self.flippedFromAllianceColor))
            ...
            SmartDashboard.putData("PhotonLocalizer", self.enabled)
        ...
        def periodic(self):
            ...
            while len(self.recentlySeenTags) > 4 * self.MAX_LOCATION_HISTORY_SIZE:
            ...
            for name, camera in self.cameras.items():
                ...
                redrawing = False
                if (now - camera.lastRedrawTime) * self.REDRAW_DASHBOARD_FREQUENCY > 1:
                ...
                while len(recentRobotLocations) > self.MAX_LOCATION_HISTORY_SIZE:
                ...
                for tag in detections.getTargets():
                    ...
                    if odometryAdjustment is None and intersectionReady:
                        odometryAdjustment = self.calculateIntersectionOdometryAdjustment(
                            camera, redrawing, robotDirection2d, robotLocation2d, tag, tagFieldPosition, flipped,
                            learningRate)
        ...
        def calculateProximityOdometryAdjustment(
            ...
            if distance > self.TAG_TOO_CLOSE_METERS:
                factor = PhotonLocalizer.TAG_TOO_CLOSE_METERS / distance
                learningRate *= factor
        ...
        def calculateIntersectionOdometryAdjustment(
            ...
            skip = False
            if abs(turnDegrees.degrees()) > self.MAX_ANGULAR_DEVIATION_DEGREES:
                skip = True
            ...
            if redrawing:
                ...
                if skip:
                    ...
                    self.fieldDrawing.getObject(lineFromTag).setPoses(
                        drawLine(10, tagLocation2d, tagLocation2d - observedVectorToTag, fraction=0.5))
                else:
                    #  - a line going from odometry position to the tag
            ...
            if distanceToTag > self.TAG_TOO_CLOSE_METERS:
                factor = self.TAG_TOO_CLOSE_METERS / distanceToTag
                learningRate *= factor
                # ^^ power 1, because the biggest cause will be systematic errors (uncalibrated camera angle or location)
            ...
            shift = shiftMeters * learningRate
            turn = Rotation2d(
                0.0) if self.TRUST_GYRO_COMPLETELY else turnDegrees * learningRate * 0.25  # 0.25 because we still kind of trust the gyro
            return shift, turn
