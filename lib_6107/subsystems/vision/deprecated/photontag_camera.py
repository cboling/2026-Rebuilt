...
class PhotonTagCamera(Subsystem):
    def __init__(self, name: Optional[str] = "photoncamera") -> None:
        super().__init__()
    ...
    def setOnlyTagIds(self, onlyTagIds=()):
        ...
        if onlyTagIds:
            logger.info(f"camera {self.name} only looking at tags {onlyTagIds}")
        else:
            logger.info(f"camera {self.name} looking at ALL tags")
    ...
    def periodic(self) -> None:
        ...
        if heartbeating != self.heartbeating:
            logger.warning(f"Camera {self.name} is " + ("UPDATING" if heartbeating else "NO LONGER UPDATING") + (
                "" if self.camera.isConnected() else " and IS NOT CONNECTED"))
        self.heartbeating = heartbeating


class PhotonTagCameraSim(Subsystem):
    def __init__(self, name: str, aprilTagFieldLayout, cameraLocationOnDrivetrain: Pose2d, drivetrain):
        """
        ...
        self.name = name or "photoncamera-sim"
        self.onlyTagIds = None
    ...
    def periodic(self) -> None:
        ...
        cameraPose: Pose2d = self.drivetrain.pose
